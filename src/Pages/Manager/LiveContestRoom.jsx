...
import { 
    Check, X, Eye, LoaderCircle, XCircle, PlayCircle, 
    Lock, Trophy, AlertTriangle, ArrowLeft, Users, CheckSquare, Clock 
} from 'lucide-react';


// --- ส่วนที่ 2: Sub-components ---

// [อัปเดต] ทำให้ StatCard เป็นปุ่มที่คลิกได้ และรับ prop `isActive`
...
const LiveContestRoom = () => {
    ...
    // [อัปเดต] State ใหม่สำหรับจัดการ Filter
    ...
    const ContestDashboard = () => {
        ...
        return (
            <div>
                <button onClick={handleBackToList} className="flex items-center gap-2 text-gray-600 hover:text-black mb-4"><ArrowLeft size={18}/> กลับไปหน้ารายการ</button>
            ...
            <div className="pt-16 p-4 sm:p-8 w-full">
                {selectedContest ? <ContestDashboard /> : (
                    <>
                        ...
                        {loading ? <div className="flex justify-center p-10"><LoaderCircle className="animate-spin text-purple-500" size={32}/></div> : (
            ...
            <Modal isOpen={detailModalOpen} onRequestClose={() => setDetailModalOpen(false)} style={{ overlay: { zIndex: 1050 } }} className="fixed inset-0 flex items-center justify-center p-4">
                {submissionInModal && (
                    <div className="bg-white rounded-lg p-6 max-w-lg w-full relative">
                        <button onClick={() => setDetailModalOpen(false)} className="absolute top-3 right-3"><XCircle /></button>
                        <h2 className="text-2xl font-bold mb-4">{submissionInModal.fish_name}</h2>
                        <div className="space-y-4 mb-4">
                            <div>
                                <h3 className="font-semibold">รูปภาพ:</h3>
                                <div className="flex flex-wrap gap-2 bg-gray-100 p-2 rounded-lg">
                                    {submissionInModal.fish_image_urls?.map((url, index) => <img key={index} src={url} className="w-28 h-28 object-cover rounded-md"/>)}
                                </div>
                            </div>
                            {submissionInModal.fish_video_url && (
                                <div>
                                    <h3 className="font-semibold">วิดีโอ:</h3>
                                    <video src={submissionInModal.fish_video_url} controls className="w-full rounded-lg"/>
                                </div>
                            )}
                        </div>
                        <div className="space-y-2">
                           <p><strong>เจ้าของ:</strong> {submissionInModal.owner?.first_name}</p>
                           <p><strong>ประเภท:</strong> {submissionInModal.fish_type}</p>
                        </div>
                    </div>
                )}
            </Modal>
