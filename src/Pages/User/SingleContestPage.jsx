// D:\ProJectFinal\Lasts\my-project\src\Pages\User\SingleContestPage.jsx
// (นี่คือไฟล์ใหม่ที่คุณต้องสร้าง)
...
const SingleContestPage = () => {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      setError(null);
      try {
        // Logic การดึงข้อมูลยังคงเหมือนเดิม เพราะใช้ Endpoint เดียวกัน
        const response = await apiService.get(`/public/content/${id}`);
        
        // [ปรับปรุง] เพิ่มการตรวจสอบว่า Content ที่ได้มาเป็น 'การประกวด' จริงหรือไม่
        if (response.data && response.data.category !== 'การประกวด') {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <p className="text-lg text-gray-600">กำลังโหลดข้อมูลการประกวด...</p>
      </div>
    );
  }
  ...
  if (error || !content) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-center p-4">
        ...
        <p className="text-gray-700 mb-6">{error || "ไม่พบข้อมูลการประกวด"}</p>
        {/* [แก้ไข] เปลี่ยน Link ให้กลับไปหน้า /contest */}
        <Link to="/contest" className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
          ← กลับไปหน้ารายการประกวด
        </Link>
      </div>
    );
  }
          ...
          <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="mt-8 pt-4 border-t">
                {/* [แก้ไข] เปลี่ยน Link ให้กลับไปหน้า /contest */}
                <Link to="/contest" className="text-purple-600 hover:underline">
                  ← กลับไปหน้ารายการประกวด
                </Link>
              </div>
            </div>
...
// [แก้ไข] เปลี่ยนชื่อ Export ให้ถูกต้อง
